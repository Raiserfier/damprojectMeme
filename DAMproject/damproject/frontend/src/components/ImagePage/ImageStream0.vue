      ...
      <div v-for="img in waterfallList"
          ...
          <div class="icons"><!-- 三个icon按钮 -->
              <ul @mouseover="enterul($event)" @mouseout="leaveul($event)">
                ...
                <li><a href="" class="icon style2 fa-info disabled" data-poptrox="iframe,1200x800"><span class="label">ForMore</span></a></li>
              </ul>
          </div>
        <div class="labels">
  ...
  import mouse_hover from "../../../static/js/userpage.js";
    export default {
        ...
        methods:{
            ...
            //搜索结果
            get_img(t,k){
                this.$api.post('/get_user_image',{key:k,email:this.my_id,user_email:this.my_id,type:t}).then(response=>{
                    //console.log(response.data);
                    this.imgList = response.data.imgList;
                    this.count += response.data.imgList.length;
                    this.last += response.data.imgList.length;
                }),(response)=>{
                    //console.log("error");
            ...
            //看主页和收藏
            get_user(id,t,k){
                this.$api.post('/get_user_image',{key:k,email:id,type:t,user_email:this.my_id}).then(response =>{
                    //console.log(response.data);
                    this.imgList = response.data.imgList;
                    this.count += response.data.imgList.length;
                }),(response)=>{
